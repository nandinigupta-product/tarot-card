/* deck.js
  Standard 78-card deck (22 Major + 56 Minor) with happy/light meanings.
  Image convention:
    - Majors: ./assets/cards/maj_00_the_fool.webp ... ./assets/cards/maj_21_the_world.webp
    - Minors: ./assets/cards/min_wands_ace.webp ... ./assets/cards/min_pentacles_king.webp

  If you already have your own deck JSON, you can override by defining:
    window.CUSTOM_DECK = [ { id, name, lightUpright, lightReversed, keywords:[], image }, ... 78 ]
...
(function () {
  ...
  // Happy/light one-paragraph meanings (upright + gentle reversed)
  const MAJOR_LIGHT = {
    "The Fool": {
  ...
  // Fallback if a major meaning is missing
  const DEFAULT_MAJOR_UP =
    "A meaningful message arrives: trust your path and take a gentle step forward.";
  const DEFAULT_MAJOR_REV =
    "A softer pace helps. Give yourself space to reset and choose again kindly.";
  ...
  const gentleReverse = (text) =>
    "Gentle note: slow down and soften. " + String(text || "").replace(/^(A|An|The)\b/i, "A");
  ...
  function buildStandardDeck() {
    ...
    MAJORS.forEach((name, i) => {
      ...
      deck.push({
        ...
        suit: null,
        rank: null,
        lightUpright: MAJOR_LIGHT[name]?.up || DEFAULT_MAJOR_UP,
        lightReversed: MAJOR_LIGHT[name]?.rev || DEFAULT_MAJOR_REV,
        keywords:
          name === "The Sun"
            ? ["joy", "confidence", "visibility"]
            : name === "The Star"
  ...
  // Allow user override with a custom deck
  if (Array.isArray(window.CUSTOM_DECK) && window.CUSTOM_DECK.length) {
    window.TAROT_DECK = window.CUSTOM_DECK;
  } else {
    window.TAROT_DECK = buildStandardDeck();
  }
})();
